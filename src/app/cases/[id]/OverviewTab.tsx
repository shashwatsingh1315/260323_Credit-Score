                ...
                <form action={async (formData) => { await handleChangePersona(formData); setShowPersonaChange(false); setPersonaReceipt(true); }} className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    <label className="flex flex-col gap-1 text-xs text-muted-foreground">
                      Customer persona
                      <select name="customerPersonaId" defaultValue={cycle.customer_persona_id || ''} className="flex h-9 w-52 rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm">
                    ...
                    <label className="flex flex-col gap-1 text-xs text-muted-foreground">
                      Contractor persona
                      <select name="contractorPersonaId" defaultValue={cycle.contractor_persona_id || ''} className="flex h-9 w-52 rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm">
                        <option value="">Default (no persona)</option>
                        {(coreData.policyOptions?.personas || []).map((p: any) => (
                          <option key={p.id} value={p.id}>{p.name}</option>
                        ))}
              ...
              <CardContent className="p-4">
                <form action={handleSelectiveUnlock} className="space-y-3" onSubmit={() => setShowUnlock(false)}>
                  ...
                  <div className="flex gap-2 mb-2">
                    <select name="section" className="flex h-9 w-52 rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm">
              ...
              <CardContent className="p-4">
                <form action={handleCounterOffer} className="space-y-3" onSubmit={() => setShowCounterOffer(false)}>
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <Input type="number" name="compositeDays" placeholder="New Composite Days" className="h-9 w-52" required max={cycle?.approved_credit_days} />
