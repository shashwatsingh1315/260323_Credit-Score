...
function TaskCompleteForm({ task, c, data, sharedStartTransition, isAnyPending }: { task: any, c: any, data: any, sharedStartTransition: any, isAnyPending: boolean }) {
  ...
  return (
    ...
    >
      ...
      <Button type="submit" size="sm" className="shrink-0" disabled={localPending || isAnyPending}>
        {localPending ? 'Saving...' : 'Complete'}
      </Button>
    </form>
  );
}

export default function CaseWorkspace({ data, initialActiveRole = 'viewer' }: CaseWorkspaceProps) {
  const c = data.case;
                          ...
                          {st.map((task: any) => (
                            <div key={task.id} className="flex items-center gap-3 py-2.5">
                              <div className="flex-1 min-w-0">
                                ...
                                <TaskCompleteForm task={task} c={c} data={data} sharedStartTransition={startTransition} isAnyPending={isPending} />
                              )}
                            </div>
                          ))}
                        </div>
